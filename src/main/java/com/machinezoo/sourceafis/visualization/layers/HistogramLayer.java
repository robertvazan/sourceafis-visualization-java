...
public record HistogramLayer(BlockGrid grid, HistogramCube histogram) implements FragmentRenderer {
	...
	public FragmentVisualization render() {
		...
		for (var at : IntPoints.stream(grid.blocks())) {
			...
			poly.add(center.x() - stretch, bottom);
			buffer.add(poly.element().fill("green").fillOpacity(0.4).stroke("#080").strokeWidth(0.2));
		}

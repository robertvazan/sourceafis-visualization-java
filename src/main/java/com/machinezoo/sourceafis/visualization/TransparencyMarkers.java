...
public class TransparencyMarkers {
	...
	private static DomContent markHistogram(HistogramCube histogram, BlockMap blocks, BlockGrid grid) {
		...
		int slots = 32;
		for (IntPoint at : IntPoints.stream(grid.blocks())) {
			...
			for (int i = 0; i < slots; ++i) {
				...
				points.add(createPolyPoint(center.x() + stretch * (2 * i + 1 - slots) / (slots - 1), bottom - height));
			}
			points.add(createPolyPoint(center.x() + stretch, bottom));
			points.add(createPolyPoint(center.x() - stretch, bottom));

...
public record ThinnedSkeletonVisualizer(SkeletonType skeleton) implements VectorVisualizer {
	@Override
	public ThinnedSkeletonKey key() {
	...
	public VectorImage visualize(TransparencyArchive archive) {
		...
		return new VectorBuffer(matrix.size())
			.background(archive)
			.embed(new SkeletonLayerImage(matrix))
			.render();
	}
}
